...
export default class CardLayout extends React.PureComponent {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    if (
      !isEqual(nextProps.location.pathname, prevState.pathname) ||
      !isEqual(nextProps.global.flatMenu, prevState.flatMenu)
    ) {
      return {
  ...
  getSelectedMenuKeys = () => {
    ...
    const selectMenu = CardLayout.getMeunMatchKeys(
      global.flatMenu,
      pathname
    )[0];
                ...
                <Dropdown
                  overlay={
                    <Menu onClick={this.onTabsActions}>
                      <Menu.Item>
                        <Link to="/sign/login">退出</Link>
                      </Menu.Item>
                    </Menu>
                  }
                >
                  <Button type="primary" ghost>
                    {this.state.user ? this.state.user.userName : '操作'}
                    <DownOutlined />
                  </Button>
                </Dropdown>
              </div>
