...
export default class TabsLayout extends React.PureComponent {
  ...
  componentWillMount (){
    //TABS模式首页默认加载'/dashboard'
    if(this.state.activeKey!==homeKey){
     this.onRemoveAll() 
    }
  ...
  onRemoveOther = () => {
    let { activeKey, panes } = this.state;
    const newpanes = panes.filter(pane => pane.key === activeKey || pane.key === homeKey);
    this.setState({ panes: newpanes });
  };

  onRemoveAll = () => { 
    this.setState({ panes: [], activeKey: homeKey,noMatch:false  }); 
    this.props.history.push(homeKey); 
  };
  ...
  render() {
    ...
    console.log(panes, activeKey, noMatch)
    return (
            ...
            {noMatch ? (
              ...
              >
                {panes.map(item => (
                  ...
                  closable={item.key===homeKey?false:true}>
                    {item}
                  </TabPane>
                ))}
              </Tabs>
